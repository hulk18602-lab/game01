...
export class MonsterSpriteRenderer {
  ...
  #draw(
    ...
    if (asset?.loaded && !definition.procedural) {
      context.drawImage(
        image,
        source.x,
        source.y,
  ...
  #clip(definition: MonsterVisualDefinition, state: MonsterAnimationState): AnimationClip {
    const asset = this.assets.get(definition.atlasUrl);
    const frames = asset?.loaded
      ? asset.metadata.animations[state]
      : proceduralAnimationFrames[state];
...
const proceduralAnimationFrames: Readonly<Record<MonsterAnimationState, readonly number[]>> = {
  idle: [0, 1, 2, 3],
  walk: [4, 5, 6, 7],
  attack: [8, 9, 10, 11],
  hit: [12, 13, 14, 15],
